...
[The Theory →](https://github.com/MettaMazza/Smithian-Fold-Theory-Of-Everything) · [Technical Documentation →](https://github.com/MettaMazza/UnisonAI/blob/main/omni/README.md)
...
1. It **composes** an answer from its conversational foundation — retrieving on-topic spans and recombining them, scored against its own zero-parameter coherence critic. Never a verbatim replay.
...
3. A local teacher model supplies the ideal response; it is held as an orbit and the couplings/spans it touches are reinforced.
4. The next time, Unison composes a stronger answer of its own — it learned *from* the correction, it did not memorise it.
...
The full architecture — every module, mechanism, forced constant, and a candid list of what works, what is stubbed, and what is known-broken — is documented, in depth, in **[`omni/README.md`](https://github.com/MettaMazza/UnisonAI/blob/main/omni/README.md)**. Nothing is hidden.

---

## Status

UnisonAI is under active development. It learns by absorbing clean, teacher-corrected experience — fluency grows with what it is taught, driven by learning alone rather than any parameter to turn, and the improvement between sessions is the finding. Its live conversational generation is **retrieve + recombine composition** over a conversational foundation — never verbatim recall — and it is the open workfront: under an independent model judge (now the project's only conversational scoreboard) the composed replies are not yet rated coherent-and-relevant, and the honest record of that measurement is kept in [`omni/README.md`](https://github.com/MettaMazza/UnisonAI/blob/main/omni/README.md). The integer-Walsh "fold eye" and "fold ear" of the paper are built (top-32 Parseval-certified coefficient bands); hearing also falls back to Whisper transcription. The separate exact-fractional per-character/word predictor is the arm that won the held-out cross-entropy benchmark above.
