...
function hl(src) {
    let word, n, out, i, c, sstart, start, d;
    n = src.length;
    out = "";
    i = 0;
    while ((i < n)) {
...
function editor_update() {
    let g, layer, lines, ta, src, i, gutter;
...
function run_code() {
    let ta, status, t0, src, r, out, t1, ms, reflow;
    ta = document.getElementById("code");
    src = ta.value;
    out = document.getElementById("output");
    status = document.getElementById("run-status");
...
function term_save() {
    let blob, a, opts, code, lines, url;
