...
function hl(src) {
    let out, d, word, sstart, start, n, i, c;
    n = src.length;
    out = "";
    i = 0;
    while ((i < n)) {
...
function editor_update() {
    let src, ta, gutter, lines, g, i, layer;
...
function run_code() {
    let ms, src, ta, t0, status, out, t1, r, reflow;
    ta = document.getElementById("code");
    src = ta.value;
    out = document.getElementById("output");
    status = document.getElementById("run-status");
...
function term_keydown(ev) {
    let sp, trimmed, inp, src, r, epp, val;
    inp = document.getElementById("term-input");
    if ((ev.key === "Enter")) {
        if (ev.shiftKey) {
            return 0;
...
function term_save() {
    let opts, code, a, url, lines, blob;
