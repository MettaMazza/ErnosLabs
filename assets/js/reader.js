...
function render_catalog() {
    let cards0, i, s, intro, cards, works, cat, sections, html;
    works = catalog();
...
function open_work(id) {
    let w, toc0, url, dl, doc, rd, rtitle;
    w = find_work(id);
    if (!w) {
        return 0;
    }
...
function build_toc(text) {
    let heads, out, i, h, cls, links, toc;
...
function do_search(ev) {
    let marks, q, re, count, doc, esc, hl;
...
function toggle_tts(ev) {
    let doc, voice_sel, text, btn, u, voice;
    if (window.ttsOn) {
        stop_tts();
        return 0;
    }
