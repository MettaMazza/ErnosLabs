...
function tokenize(src) {
    let i, indent, ln, trimmed, lines, norm, raw, line, toks;
...
function parse_set(P, ln) {
    let is_field, i, t, toks, rest, target_name, ts, ex, nd, field;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let ln, pair, cond, cond_toks, nd, econd, second, ct, h, toks, body, ts, ts2, ebody, nx;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let name, body, params, nd, i, t, toks;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let i, name, b, callee, fnv, fn, args;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let i, fn, res, ran_main, genv, P, lines, program, st;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let v, st, n, P, genv, program, i, res, lines;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
