...
function tokenize(src) {
    let trimmed, indent, raw, i, toks, lines, norm, line, ln;
...
function parse_set(P, ln) {
    let rest, ex, toks, nd, target_name, is_field, i, ts, t, field;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let ct, cond, econd, body, h, nd, ts, toks, second, cond_toks, ebody, pair, ts2, ln, nx;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let toks, i, body, name, t, nd, params;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let i, callee, name, args, b, fn, fnv;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let ran_main, i, fn, P, program, genv, lines, st, res;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let program, i, lines, n, genv, st, v, P, res;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
