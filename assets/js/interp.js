...
function tokenize(src) {
    let lines, raw, trimmed, toks, i, line, ln, norm, indent;
...
function parse_set(P, ln) {
    let target_name, t, nd, rest, field, toks, is_field, i, ts, ex;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let ln, body, ebody, nx, toks, ts, cond, second, nd, cond_toks, h, ct, ts2, pair, econd;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let body, nd, name, toks, t, params, i;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let args, name, fn, b, fnv, callee, i;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let res, P, genv, ran_main, st, fn, i, lines, program;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let genv, v, program, st, res, P, lines, i, n;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
