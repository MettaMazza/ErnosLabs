...
function tokenize(src) {
    let norm, indent, ln, lines, i, trimmed, line, raw, toks;
...
function parse_set(P, ln) {
    let t, i, target_name, rest, ts, toks, nd, is_field, field, ex;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let h, second, ln, ebody, body, nx, cond, toks, econd, nd, cond_toks, pair, ct, ts2, ts;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let i, t, name, toks, body, nd, params;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let i, fnv, name, callee, b, fn, args;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let P, genv, ran_main, res, lines, program, i, fn, st;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let i, st, genv, lines, P, n, res, program, v;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
