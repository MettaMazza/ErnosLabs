...
function tokenize(src) {
    let norm, lines, i, ln, indent, toks, line, trimmed, raw;
...
function parse_set(P, ln) {
    let ts, ex, is_field, field, nd, toks, i, t, target_name, rest;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let pair, ts2, toks, ebody, ct, cond_toks, second, h, body, cond, nd, nx, econd, ln, ts;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let body, nd, name, toks, t, i, params;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let i, callee, args, b, fn, fnv, name;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let program, res, lines, fn, genv, P, ran_main, i, st;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let P, lines, genv, i, st, res, n, program, v;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
