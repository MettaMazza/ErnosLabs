...
function tokenize(src) {
    let toks, norm, i, lines, line, indent, ln, trimmed, raw;
...
function parse_set(P, ln) {
    let rest, ts, nd, target_name, is_field, toks, field, ex, t, i;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let cond, nx, ts2, h, second, ct, ts, body, ln, econd, ebody, nd, pair, cond_toks, toks;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let params, t, i, name, body, nd, toks;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let i, fn, args, fnv, name, callee, b;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let P, res, program, lines, i, st, genv, ran_main, fn;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let P, res, v, program, lines, n, i, st, genv;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
