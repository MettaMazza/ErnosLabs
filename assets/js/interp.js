...
function tokenize(src) {
    let indent, raw, toks, norm, lines, ln, i, line, trimmed;
...
function parse_set(P, ln) {
    let ts, target_name, toks, i, t, ex, nd, rest, is_field, field;
    toks = ln.toks;
    i = 1;
...
function parse_if(P, indent) {
    let cond, cond_toks, ln, ebody, ct, ts, second, body, toks, nx, nd, econd, ts2, pair, h;
    ln = cur_line(P);
    toks = ln.toks;
...
function parse_define(P, indent, ln) {
    let t, body, nd, name, toks, params, i;
    toks = ln.toks;
    name = toks[1].val;
    params = [];
    i = 2;
...
function eval_call(x, e) {
    let b, name, args, fn, fnv, i, callee;
    callee = x.callee;
    args = [];
    i = 0;
...
function ep_run(src) {
    let P, res, fn, i, st, program, lines, genv, ran_main;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    lines = tokenize(src);
...
function ep_repl(src) {
    let v, genv, P, n, st, program, i, lines, res;
    res = JSON.parse("{}");
    window.epOut = [];
    window.epErr = false;
    if (!window.epReplEnv) {
