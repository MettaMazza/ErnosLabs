...
// `repo` is kept only as provenance (where the weights originally came from) and
// is used as the Hugging Face fallback link when the machine is offline.
//
// This list is reconciled against what the source machine actually serves —
// only models with complete, finalized weights on disk are listed here.
...
window.AI_STATS = { count: 35, size: "1.9 TB", note: "open weights preserved" };
...
window.AI_RUNNERS = [
  ...
  { name: "ComfyUI", what: "Node-based runner for image & video models.", url: "https://github.com/comfyanonymous/ComfyUI", prog: "programs/ComfyUI" },
  { name: "ComfyUI-GGUF", what: "Run quantised image/video models in ComfyUI.", url: "https://github.com/city96/ComfyUI-GGUF", prog: "" },
...
window.AI_SECTIONS = [
  ...
  { key: "video", heading: "Video generation", sub: "Open text-to-video — rare, and easy to lose. Preserved runnable." },
...
const CGGUF = _r("ComfyUI-GGUF", "https://github.com/city96/ComfyUI-GGUF");
...
window.AI_MODELS = [
  ...
  { id: "flux-1-dev", name: "FLUX.1-dev", cat: "image", size: "6.3 GB", fmt: "GGUF · Q4_K_S", path: "models/Creative_Models/flux1-dev-Q4_K_S.gguf", repo: "city96/FLUX.1-dev-gguf", license: "FLUX.1-dev (non-commercial)", runner: CGGUF, cmd: "Load flux1-dev-Q4_K_S.gguf with the ComfyUI-GGUF unet loader", desc: "Black Forest Labs' flow model — near-frontier open image quality." },

  // ---- Video -----------------------------------------------------------
  { id: "hunyuan-video", name: "HunyuanVideo T2V 720p", cat: "video", size: "24 GB", fmt: "GGUF · BF16", path: "models/Creative_Models/hunyuan-video-t2v-720p-BF16.gguf", repo: "city96/HunyuanVideo-gguf", license: "Tencent Hunyuan Community", runner: CGGUF, cmd: "Load hunyuan-video-t2v-720p-BF16.gguf with the ComfyUI-GGUF loader", desc: "Tencent's open text-to-video model at 720p — one of the strongest openly-downloadable video generators." },
  { id: "ltx-video-2b", name: "LTX-Video 2B v0.9", cat: "video", size: "3.7 GB", fmt: "GGUF · BF16", path: "models/Creative_Models/ltx-video-2b-v0.9-BF16.gguf", repo: "city96/LTX-Video-gguf", license: "LTX open-weights", runner: CGGUF, cmd: "Load ltx-video-2b-v0.9-BF16.gguf with the ComfyUI-GGUF loader", desc: "Lightricks' compact, fast open text-to-video — real-time-ish generation on a single GPU." },

  // ---- GGUF LLMs (small → giant) --------------------------------------
  ...
  { id: "gemma-4-e2b", name: "Gemma-4-E2B-it", cat: "llm", size: "2.9 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/gemma-4-E2B-it-Q4_K_M.gguf", repo: "unsloth/gemma-4-E2B-it-GGUF", license: "Gemma", runner: LLAMA, cmd: "llama-cli -m gemma-4-E2B-it-Q4_K_M.gguf -p \"Hello\"", desc: "The tiniest Gemma-4 — an efficient ~2B-class model that runs almost anywhere." },
  { id: "qwen3.5-4b", name: "Qwen3.5-4B", cat: "llm", size: "2.6 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Qwen3.5-4B-Q4_K_M.gguf", repo: "unsloth/Qwen3.5-4B-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen3.5-4B-Q4_K_M.gguf -p \"Hello\"", desc: "Alibaba's pocket-sized Qwen3.5 — small enough for a phone-class device." },
  { id: "qwen3.5-9b", name: "Qwen3.5-9B", cat: "llm", size: "5.3 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Qwen3.5-9B-Q4_K_M.gguf", repo: "unsloth/Qwen3.5-9B-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen3.5-9B-Q4_K_M.gguf -p \"Hello\"", desc: "Alibaba's compact Qwen3.5 — a capable 9B for a single GPU." },
  { id: "phi-4", name: "Phi-4", cat: "llm", size: "8.3 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/phi-4-Q4_K_M.gguf", repo: "unsloth/phi-4-GGUF", license: "MIT", runner: LLAMA, cmd: "llama-cli -m phi-4-Q4_K_M.gguf -p \"Solve:\"", desc: "Microsoft's 14B — a small model tuned hard for reasoning and maths." },
  ...
  { id: "qwen3.6-27b", name: "Qwen3.6-27B", cat: "llm", size: "16 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Qwen3.6-27B-Q4_K_M.gguf", repo: "unsloth/Qwen3.6-27B-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen3.6-27B-Q4_K_M.gguf -p \"Hello\"", desc: "Alibaba's mid-size Qwen3.6 dense model." },
  ...
  { id: "qwen3.6-35b-a3b", name: "Qwen3.6-35B-A3B", cat: "llm", size: "21 GB", fmt: "GGUF · UD-Q4_K_M", path: "models/GGUF_Models/Qwen3.6-35B-A3B-UD-Q4_K_M.gguf", repo: "unsloth/Qwen3.6-35B-A3B-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen3.6-35B-A3B-UD-Q4_K_M.gguf -p \"Hello\"", desc: "Qwen3.6 as a mixture-of-experts — 35B total, ~3B active per token (Unsloth dynamic quant)." },
  ...
  { id: "qwen3-coder-next", name: "Qwen3-Coder-Next", cat: "llm", size: "45 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Qwen3-Coder-Next-Q4_K_M.gguf", repo: "unsloth/Qwen3-Coder-Next-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen3-Coder-Next-Q4_K_M.gguf -p \"write a function\"", desc: "Alibaba's coding-specialised Qwen3, the 'Next' iteration — quantised to run locally." },
  ...
  { id: "llama-4-scout", name: "Llama-4-Scout-17B-16E", cat: "llm", size: "61 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Llama-4-Scout", dir: true, repo: "unsloth/Llama-4-Scout-17B-16E-Instruct-GGUF", license: "Llama 4 Community", runner: LLAMA, cmd: "llama-cli -m Llama-4-Scout-17B-16E-Instruct-Q4_K_M-00001-of-00002.gguf -p \"Hello\"", desc: "Meta's Llama 4 Scout — 17B active across 16 experts, long-context, runs on a strong machine." },
  { id: "qwen3-235b", name: "Qwen3-235B-A22B (2507)", cat: "llm", size: "133 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Qwen3-235B", dir: true, repo: "unsloth/Qwen3-235B-A22B-Instruct-2507-GGUF", license: "Apache-2.0", runner: LLAMA, cmd: "llama-cli -m Qwen_Qwen3-235B-A22B-Instruct-2507-Q4_K_M-00001-of-00004.gguf -p \"Hello\"", desc: "Alibaba's Qwen3-235B-A22B Instruct (2507) — 235B total, 22B active per token." },
  { id: "deepseek-v4-flash", name: "DeepSeek-V4-Flash", cat: "llm", size: "145 GB", fmt: "GGUF · MXFP4", path: "models/GGUF_Models/DeepSeek-V4-Flash", dir: true, repo: "unsloth/DeepSeek-V4-Flash-GGUF", license: "MIT", runner: LLAMA, cmd: "llama-cli -m DeepSeek-V4-Flash-MXFP4-00001-of-00004.gguf -p \"Think:\"", desc: "DeepSeek's V4 Flash — a large open mixture-of-experts model, MXFP4-quantised to fit a big rig." },
  { id: "llama-4-maverick", name: "Llama-4-Maverick-17B-128E", cat: "llm", size: "226 GB", fmt: "GGUF · Q4_K_M", path: "models/GGUF_Models/Llama-4-Maverick", dir: true, repo: "unsloth/Llama-4-Maverick-17B-128E-Instruct-GGUF", license: "Llama 4 Community", runner: LLAMA, cmd: "llama-cli -m Llama-4-Maverick-17B-128E-Instruct-Q4_K_M-00001-of-00005.gguf -p \"Hello\"", desc: "Meta's Llama 4 Maverick — 17B active across 128 experts; frontier-scale open MoE." },
  ...
  { id: "glm-5.2", name: "GLM-5.2 (744B)", cat: "llm", size: "434 GB", fmt: "GGUF · Q4", path: "models/GGUF_Models/GLM-5.2-Q4", dir: true, repo: "unsloth/GLM-5.2-GGUF", license: "see source", runner: LLAMA, cmd: "llama-cli -m GLM-5.2-Q4_K_M-00001-of-*.gguf -p \"Hello\"", desc: "Zhipu's GLM-5.2 — a 744B-parameter frontier-scale open mixture-of-experts model." },
];
